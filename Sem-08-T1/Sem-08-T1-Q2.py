...
def main():
    ...
    mes = int(input('Digite um mes: ').strip())
    ano = int(input('Digite um ano: ').strip())

    d_atual = int(input('Digite outro dia: ').strip())
    m_atual = int(input('Digite outro mes: ').strip())
    a_atual = int(input('Digite outro ano: ').strip())
   
    dia_mais_recente,mes_mais_recente,ano_mais_recente = verifica_idade(dia,mes,ano,d_atual,m_atual,a_atual)

    print(f'Das datas digitadas a mais recente é: {dia_mais_recente}/{mes_mais_recente}/{ano_mais_recente}')
